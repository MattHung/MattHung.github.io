 ...
 */
function ConvertJsonToTable(parsedJson, tableId, tableClassName, linkText, Editable)
{
    ...
    if(Editable)
        tdRow = '<td><div contenteditable>{0}</div></td>';
...
function ConvertTableToJson(){
    ...
    var $rows = $("tbody tr").each(function(index) {
      ...
      $cells.each(function(cellIndex) {
        ...
        text = text.replace('<div contenteditable="">', "");
        text = text.replace("</div>", "");

        text = text.replace("<i>", "");
        text = text.replace("</i>", "");

        total_rows[index][$($headers[cellIndex]).html()] = text;
      });    
    });

    return JSON.stringify(total_rows);
}