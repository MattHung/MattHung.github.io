...
CocosWidget.getChild = function(node, name){
...
CocosWidget.getNode =function(parent_node, path){
    ...
    for(var i =0; i<path_names.length; i++)
    {
        ...
        if(node)
        if(node.__instanceId==parent_node.__instanceId)
            node = CocosWidget.getChild(parent_node, path_names[i]);
...
CocosWidget.cloneNode =function(node) {
    var cloneNodeInternal = function(internalNode){
        ...
        if(internalNode instanceof ccui.TextField){
            result = new ccui.TextField();
            result.setString(internalNode.getString());
            result.setFontSize(internalNode.getFontSize());
            result.setFontName(internalNode.getFontName());
            result.setColor(internalNode.getColor());
            result.setPlaceHolder(internalNode.getPlaceHolder());
            result.setAnchorPoint(internalNode.getAnchorPoint());
            result.setContentSize(internalNode.getContentSize());

            // result.setTextHorizontalAlignment(internalNode.getTextHorizontalAlignment());
            // result.setTextVerticalAlignment(internalNode.getTextVerticalAlignment());
        }
