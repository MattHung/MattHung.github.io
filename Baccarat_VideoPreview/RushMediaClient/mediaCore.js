...
WSAvcPlayer = function(canvas_name){
  ...
  {      
      ...
      this.ctx.fillText("CCU: " + String(this.ccu),10, 200);
  }.bind(this);
  ...
  this.decoder.onPictureDecoded = function(buffer, width, height, infos){
    ...
    if(this.RenderingCallback){
        this.RenderingCallback(this.canvas, width, height, this.decoder.canvasBuffer);
        return;
    }
  }.bind(this);
  ...
  this.connect = function(url){
    ...
    this.ws.onerror = function(error){this.onwserror(error)}.bind(this);
    this.ws.onclose = function(close){this.onwsclose(close)}.bind(this);
